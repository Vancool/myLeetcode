...
class Solution(object):
    def buildTree(self, preorder, inorder):
        ...
        for i, num in enumerate(inorder):
            hashmap[num] = i
        self.pre_count = 0
        def buildTree(preorder, inleft, inright, inorder):
            if self.pre_count >= len(preorder):
                return None
            if inleft > inright:
                return None
            root = TreeNode(preorder[self.pre_count])
            root_inorder_index = hashmap[preorder[self.pre_count]]
            self.pre_count += 1
            root.left = buildTree(preorder, inleft, root_inorder_index-1, inorder)
            root.right = buildTree(preorder, root_inorder_index+1, inright, inorder)
            return root
        return buildTree(preorder, 0, len(inorder)-1, inorder)
